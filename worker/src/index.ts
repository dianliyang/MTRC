...
app.use("/api/*", async (c, next) => {
  ...
  if (
    ...
    c.req.path === "/api/confirm-join" ||
    c.req.path === "/api/accept-invitation"
  )
    return next();
...
async function sendEmail(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error(
        "Note: Ensure your FROM_EMAIL domain is verified in Resend dashboard.",
      );
    }
  } catch (e) {
  ...
  try {
    ...
    if (emails.length > 0) {
      await sendEmail(
        c.env,
        emails.join(","),
        `New Book: ${updated.title}`,
        `We are reading ${updated.title}`,
      );
    }
    // Signal
    if (phones.length > 0) {
      ...
      c.executionCtx.waitUntil(
        Promise.all(phones.map((p) => sendSignalMessage(c.env, p, msg))),
      );
    }
...
app.post("/api/subscribe", async (c) => {
  try {
    ...
    if (!email && !phoneNumber)
      return c.json({ error: "Provide email or phone" }, 400);

    const existing = await db.query.subscribers.findFirst({
      where: or(
        email ? eq(schema.subscribers.email, email) : undefined,
        phoneNumber
          ? eq(schema.subscribers.phoneNumber, phoneNumber)
          : undefined,
      ),
    });
...
app.get("/api/books/:id/like-status", async (c) => {
  try {
    ...
    const existing = await db.query.likes.findFirst({
      where: and(
        eq(schema.likes.userId, parseInt(payload.id)),
...
app.post("/api/books/:id/toggle-like", async (c) => {
  try {
    ...
    if (existing) {
      ...
      await db
        .insert(schema.likes)
        .values({ userId, bookId, createdAt: new Date() });
      await db
        .update(schema.books)
        .set({
  ...
  try {
    ...
    const meetingsWithDetails = await Promise.all(
      results.map(async (m) => {
        // Get books
        const mb = await db
          .select({ bookId: schema.meetingBooks.bookId })
...
app.post("/api/meetings/:id/publish", adminMiddleware, async (c) => {
  try {
    ...
    // Invalidate cache
    c.executionCtx.waitUntil(
      purgeCache(c, ["/api/meetings", `/api/meetings/${id}`]),
    );
...
app.post("/api/meetings/:id/join", async (c) => {
  try {
    ...
    const name =
      body.name ||
      `Member_${Math.random().toString(36).substring(2, 7).toUpperCase()}`;
    ...
    c.executionCtx.waitUntil(
      (async () => {
        ...
        await sendEmail(
          c.env,
          body.email,
          `Confirm Join: ${meeting.topic}`,
          `Please confirm your spot for ${meeting.topic}`,
          html,
        );
      })(),
    );
...
app.post("/api/admin/invite", async (c) => {
  ...
  const name =
    body.name ||
    `${prefix}_${Math.random().toString(36).substring(2, 7).toUpperCase()}`;
  ...
  try {
    ...
    await sendEmail(
      c.env,
      email,
      "Invitation to MoreThan Reading Club",
      `Hello ${name}, you have been invited. Use the link to accept your invitation.`,
      html,
    );
...
app.post("/api/login", async (c) => {
  ...
  if (!user || user.deletedAt)
    return c.json({ error: "Invalid credentials or account deleted" }, 401);
  ...
  const token = await sign(
    ...
    secret,
    "HS256",
  ); // Added algorithm explicitly for clarity
...
app.post("/api/profile/password", async (c) => {
  try {
    ...
    const user = await db.query.users.findFirst({
      where: and(
        eq(schema.users.id, parseInt(payload.id)),
        eq(schema.users.password, currentHashHex),
      ),
    });
